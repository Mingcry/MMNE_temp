...
# from GCN.GCN1layer_init import GCN
from GCN.GCN2layer_init import GCN
# from GCN.GCN_init import GCN
# from GCN.GCN4layer_init import GCN
# from GCN.GCN5layer_init import GCN
...
root = 'G:/Pytorch/Moudal_tree/'

in_dim = 768
out_dim = 400
feat = 'all_modal_have'
save = root + 'Tree_' + feat + '/data/previt_train_data_' + str(out_dim) + '_2gcn.txt'
    ...
    for i in range(1, len(name_movie) + 1):
        ...
        if image_nodes[i-1]:
            for u, nod in image_nodes[i-1].items():
                ...
                A[u][i] = 1
                # print('image', u, nod)
                x[u] = torch.tensor(eval(nod)).unsqueeze(0)
        if video_nodes[i-1]:
            for u, nod in video_nodes[i-1].items():
                A[i][u] = 1
                A[u][i] = 1
...
for r in range(0, len(file)):
    ...
    for tt in data:
        tem = tt.split('\t')[1:]
        movie0 = eval(tem[2])
        if movie0[0][1] != 'None' and movie0[1][1] != 'None' and movie0[4][1] != 'None' and movie0[5][1] != 'None':
            temp.append(tt)
    ...
    for item in info:
        ...
        for j, node in enumerate(item):
            if node[1] != 'None' and j < 2:  # 文本信息j=0,1,2,3
                ...
                k += 1
                node_num += 1
            if node[1] != 'None' and j == 4:  # 图片信息j=4
                image[k] = node[1]
                k += 1
                node_num += 1
            if node[1] != 'None' and j == 5:  # 视频信息j=5
                video[k] = node[1]
                k += 1
                node_num += 1
    ...
    print('n={}'.format(n))
    with open(root+'Tree_' + feat + '/data/record_n_400.txt', 'a+') as ff:
        ff.write(f'n={n}\n')
